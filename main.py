...
STARTING_MESSAGE = os.getenv('STARTING_MESSAGE', "Solve the issue in the attached image.")

SYSTEM_PROMPT = os.getenv('SYSTEM_PROMPT', "Provide brief, accurate responses in the form of actionable steps to solve the issue in user's attached image & query.")
...
async def describe_image(image_url, user_query=None):
    try:
        ...
        response = client.responses.create(
            ...
            input=[
                ...
                },
                {
                    "role": "user",
                    "content": [
                        {"type": "input_text", "text": user_query if user_query else STARTING_MESSAGE},
                        {
                            "type": "input_image",
                            "image_url":image_url
                        },
        ...
        full_description = response.output_text
    ...
    if not CHANNEL_IDS or message.channel.id in CHANNEL_IDS:
        ...
        if REPLY_TO_LINKS:
            ...
            for image_url in image_urls:
                description_chunks = await describe_image(image_url, message.content)
